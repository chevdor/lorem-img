...
(function(scope){
  ...
  gulp.task('p-scripts:package:now', function(){
    var jsBundles = [distDir + '/scripts/lorem-img.min.js'];
  ...
  var scriptsBrowserify = function(env){
    var jsBundles = ['app/scripts/lorem-img.js'];
...
(function(scope){
  ...
  gulp.task('vulcanize:inline:now',function(){
    return gulp.src(distDir + '/lorem-img.html', {base: distDir + '/'})
      .pipe($.vulcanize({
  ...
  var vulcanizeWithExternalScriptsAndStyles = function(vulcanizeOptions){
    return gulp.src(tmpDir + '/lorem-img.html')
